...
def predict_energy_kwh(input_data, duration_minutes=5):
    try:
        ...
        print(f"✅ Model loaded with features: {features}")  # ← Debug
        
        processed = preprocess_input(input_data, scaler, features)
        print(f"✅ Processed input shape: {processed.shape}")  # ← Debug
        
        pred_power = model.predict(processed)[0]
        print(f"✅ Raw prediction: {pred_power}")  # ← Debug
        
        # Pastikan prediksi tidak negatif
        pred_power = max(0, pred_power)
        print(f"✅ Final prediction: {pred_power}")  # ← Debug
...
def predict_future_energy(start_data, duration_type='day', num_periods=1):
    try:
        ...
        for date in future_dates:
            ...
            # PERBAIKAN: Pastikan prediksi tidak negatif
            pred_power = max(0, pred_power)  # ← Tambahkan ini
...
def prepare_future_data(future_dates, last_sensor_data, numeric_features, time_features):
    ...
    for dt in future_dates:
        row = {
            # PERBAIKAN: Validasi dan pastikan nilai positif
            'voltage': abs(float(last_sensor_data.get('voltage', 220))),
            'current': abs(float(last_sensor_data.get('current', 1.5))),  # ← Pastikan positif
            'energy': abs(float(last_sensor_data.get('energy', 0))),
            'frequency': abs(float(last_sensor_data.get('frequency', 50))),
            'power_factor': min(max(float(last_sensor_data.get('power_factor', 0.9)), 0), 1),  # ← Clamp antara 0-1
            'temperature': float(last_sensor_data.get('temperature', 25)),
            'humidity': min(max(float(last_sensor_data.get('humidity', 60)), 0), 100),  # ← Clamp antara 0-100
            'hour': dt.hour,
